 ...
 * {@code ILaunchPluginService.initializeLaunch}, well after {@link io.github.intisy.nylium.core.NyliumKernel#boot}
 * returns, so {@link #addToClasspath} and {@link #whenModuleLoadable} only stash what they are given;
 * {@link NyliumMl9LaunchPlugin} drives the actual hand-off and activation once the GAME layer exists.
 */
final class Ml9Platform implements Platform {
     ...
     * but not yet at {@code onLoad}, where {@code NyliumKernel.boot} has to select a module.
     */
    @Override
    ...
    Path pendingModulePath() {
        ...
        if (path == null) {
            throw new NyliumException("beginScanning ran before NyliumKernel.boot extracted a module to hand off.");
        }
        return path;
    }
    ...
    void activate(IModuleLayerManager manager) {
        if (manager == null) {
            throw new NyliumException("No IModuleLayerManager was captured before initializeLaunch, "
                    + "so the GAME layer cannot be resolved; completeScan never ran on "
                    + "NyliumMl9TransformationService.");
        }
        try {
            ...
            if (!optional.isPresent()) {
                throw new NyliumException("The GAME module layer was not present when ModLauncher 9+ "
            ...
            throw new NyliumException("Could not resolve the GAME module layer's class loader for module '"
                    + moduleName + "'", e);
        }
