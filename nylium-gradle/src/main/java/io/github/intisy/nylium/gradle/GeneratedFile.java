 ...
 *
 * @implNote {@code nyliumUniversalJar} takes each of these from its own writer's output instead of
 *     sweeping {@code build/nylium/}. Gradle does not delete the output of a task that no longer
...
final class GeneratedFile {
    ...
    private final TaskProvider<NyliumTextFileTask> writer;

    GeneratedFile(String relativePath, TaskProvider<NyliumTextFileTask> writer) {
        this.relativePath = relativePath;
        this.writer = writer;
    }

    TaskProvider<NyliumTextFileTask> writer() {
        return writer;
    }

    Provider<RegularFile> destination() {
        return writer.flatMap(NyliumTextFileTask::getDestination);
    }
